...
const GuardiaAPI = {
    ...
    async registrarAsistencia(uid, tipo, gps, metodo, fotoBlob, dispositivo, puesto) {
        ...
        // Obtener chat_id del guardia para notificación dual
        let guardiaChat = null;
        try {
            const gu = await query('usuarios', `id=eq.${uid}&select=telegram_chat_id`);
            guardiaChat = gu?.[0]?.telegram_chat_id || null;
        } catch(e) {}
