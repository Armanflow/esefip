...
const AdminAPI = {
    ...
    async resumenDescuentos(mes, anio) {
        // Query directa más simple y confiable que la función
        const data = await query('descuentos_diarios',
            `usuario_id=neq.0&fecha=gte.${anio}-${String(mes).padStart(2,'0')}-01&fecha=lte.${anio}-${String(mes).padStart(2,'0')}-31&select=usuario_id,tipo,monto,minutos,usuarios(nombre,apellido,cedula,turno,salario_base)`
        );
        ...
        data.forEach(d => {
            ...
            if (!map[uid]) {
                map[uid] = { id: uid, ...d.usuarios, desc_atrasos:0, desc_faltas:0, desc_rondas:0, bono_relevo:0, total_descuentos:0, min_atrasos:0, dias_falta:0, rondas_perdidas:0 };
            }
    ...
    async detalleDescuentos(usuarioId, mes, anio) {
        ...
        const data = await query('descuentos_diarios',
            `usuario_id=eq.${usuarioId}&fecha=gte.${anio}-${mesStr}-01&fecha=lte.${anio}-${mesStr}-31&select=*,usuarios(nombre,apellido)&order=fecha.desc`
        );
        return (data||[]).map(d => ({...d, nombre: d.usuarios?.nombre, apellido: d.usuarios?.apellido}));
    },
