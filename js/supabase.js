...
const AdminAPI = {
    ...
    async resumenDescuentos(mes, anio) {
        ...
        const data = await query('descuentos_diarios',
            `usuario_id=neq.0&fecha=gte.${anio}-${mesStr}-01&fecha=lte.${anio}-${mesStr}-${ultimoDia}&select=usuario_id,tipo,monto,minutos,usuarios(nombre,apellido,cedula,turno,salario_base)`
        );
    ...
    async detalleDescuentos(usuarioId, mes, anio) {
        ...
        const ultimoDia = new Date(anio, mes, 0).getDate();
        const data = await query('descuentos_diarios',
            `usuario_id=eq.${usuarioId}&fecha=gte.${anio}-${mesStr}-01&fecha=lte.${anio}-${mesStr}-${ultimoDia}&select=*,usuarios(nombre,apellido)&order=fecha.desc`
        );
