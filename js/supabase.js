...
async function upsert(table, body, onConflict) {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}?on_conflict=${onConflict||'id'}`, {
        method: 'POST',
        headers: {
            'apikey': SUPABASE_KEY,
            'Authorization': `Bearer ${SUPABASE_KEY}`,
...
// ============================================================
// HELPER: obtener telegram_chat_id de un guardia
// ============================================================
async function getChatGuardia(uid) {
    try {
        const rows = await query('usuarios', `id=eq.${uid}&select=telegram_chat_id`);
...
const AdminAPI = {
    ...
    async crearGuardia(d) {
        ...
        const row = rows?.[0] ?? {};
        // Guardar puesto y geofencing por separado
        if (row.resultado > 0) {
            await update('usuarios', {
                puesto:            d.puesto||null,
                telegram_chat_id:  d.telegram_chat_id||null,
                puesto_lat:        d.puesto_lat   ? parseFloat(d.puesto_lat)   : null,
                puesto_lon:        d.puesto_lon   ? parseFloat(d.puesto_lon)   : null,
                puesto_radio:      d.puesto_radio ? parseInt(d.puesto_radio)   : 200
            }, `id=eq.${row.resultado}`);
        }
    ...
    async actualizarGuardia(d) {
        ...
        // Actualizar puesto y geofencing directamente
        await update('usuarios', {
            puesto:            d.puesto||null,
            telegram_chat_id:  d.telegram_chat_id||null,
            puesto_lat:        d.puesto_lat   ? parseFloat(d.puesto_lat)   : null,
    ...
    async listarPermisos(estado) {
        const filtro = estado ? `estado=eq.${estado}&` : '';
        const [permisos, guardias] = await Promise.all([
            query('permisos', `${filtro}order=creado_en.desc&limit=100&select=*`),
            query('usuarios', 'select=id,nombre,apellido,cedula')
        ]);
        const gMap = {};
        (guardias||[]).forEach(g => { gMap[g.id] = g; });
        return (permisos||[]).map(p => ({
            ...p,
            nombre:   gMap[p.usuario_id]?.nombre   || '',
            apellido: gMap[p.usuario_id]?.apellido || '',
            cedula:   gMap[p.usuario_id]?.cedula   || ''
        }));
    },
    async responderPermiso(id, estado, respuesta, adminId) {
        await update('permisos', {
            estado, respuesta: respuesta||'',
            admin_id: adminId, actualizado_en: new Date().toISOString()
        }, `id=eq.${id}`);
        // Notificar al guardia por Telegram
        const rows = await query('permisos', `id=eq.${id}&select=usuario_id,fecha_desde,fecha_hasta,motivo`);
        if (rows?.[0]) {
            const p = rows[0];
            const chatGuardia = await getChatGuardia(p.usuario_id);
            const ico = estado === 'aprobado' ? '✅' : '❌';
            await Telegram.sendMessage(
                `${ico} <b>PERMISO ${estado.toUpperCase()} — ESEFIP</b>
...
` : '') +
    ...
    async actualizarFotoGuardia(uid, fotoPerfil, fotoQRBanco) {
        const upd = {};
        if (fotoPerfil !== undefined) upd.foto_perfil = fotoPerfil;
        if (fotoQRBanco !== undefined) upd.foto_qr_banco = fotoQRBanco;
        await update('usuarios', upd, `id=eq.${uid}`);
        return { success: true };
    }
};
...
const GuardiaAPI = {
    ...
    async solicitarPermiso(uid, fechaDesde, fechaHasta, motivo) {
        await insert('permisos', {
            usuario_id: uid, fecha_solicitud: new Date().toISOString().split('T')[0],
            fecha_desde: fechaDesde, fecha_hasta: fechaHasta,
            motivo, estado: 'pendiente'
        });
        // Notificar al admin
        const g = await query('usuarios', `id=eq.${uid}&select=nombre,apellido,cedula,puesto`);
        const gu = g?.[0];
        await Telegram.sendMessage(
            `📋 <b>SOLICITUD DE PERMISO — ESEFIP</b>

` +
            `👤 ${gu?.nombre} ${gu?.apellido} | 🪪 ${gu?.cedula}
` +
            (gu?.puesto ? `🏢 ${gu.puesto}
` : '') +
...
` +
            `⏰ ${new Date().toLocaleString('es-ES')}
` +
            `<i>Revisar en el panel admin → Permisos</i>`
        );
        return { success: true };
    },
    async misPermisos(uid) {
        return query('permisos', `usuario_id=eq.${uid}&order=creado_en.desc&limit=20&select=*`);
    },
...
// ============================================================
const Storage = {
    async uploadFotoPerfil(uid, blob) {
        const path = `perfiles/perfil_${uid}_${Date.now()}.jpg`;
        return this.upload(path, blob, 'image/jpeg');
    },
    async uploadFotoQRBanco(uid, blob) {
        const path = `qr_banco/qr_banco_${uid}_${Date.now()}.jpg`;
        return this.upload(path, blob, 'image/jpeg');
    },
    async upload(path, blob, mimeType) {
        try {
            ...
            let uploadBlob = blob;
            if (blob.size > 500000) {
                uploadBlob = await comprimirImagen(blob, 0.7);
            }
            const controller = new AbortController();
            const timeout = setTimeout(() => controller.abort(), 30000);
            // Usar PUT con upsert para Supabase Storage (POST falla si el objeto ya existe)
            ...
            if (!res.ok) {
                ...
                // Intentar con PUT si POST falla
                if (res.status === 400 || res.status === 409) {
                    const res2 = await fetch(`${SUPABASE_URL}/storage/v1/object/esefip-uploads/${path}`, {
                        method: 'PUT',
                        headers: {
                            'apikey': SUPABASE_KEY,
                            'Authorization': `Bearer ${SUPABASE_KEY}`,
                    ...
                    if (!res2.ok) {
                        ...
                        throw new Error(`Storage ${res2.status}: ${err2}`);
                    }
                    clearTimeout(timeout);
                    console.log('[Storage] Subido con PUT OK:', path);
                    return path;
                }
            ...
            if (e.name === 'AbortError') throw new Error('Timeout al subir imagen (30s). Verifica tu conexión.');
            throw e;
        }
    },
...
async function comprimirImagen(blob, calidad = 0.7) {
...
// ============================================================
const Telegram = {
    async sendMessage(text, extraChatId) {
        const _send = async (chatId) => {
            try {
                ...
                if (!data.ok) console.warn('[Telegram] Error chat', chatId, ':', data.description);
                return data;
            } catch(e) { console.error('[Telegram] sendMessage error:', e.message); }
        };
        // Siempre enviar al admin
        await _send(parseInt(TELEGRAM_CHAT));
        // Si hay chat_id extra del guardia y es diferente al admin, enviar también
        if (extraChatId && String(extraChatId) !== String(TELEGRAM_CHAT)) {
            await _send(parseInt(extraChatId));
        }
    },
    async sendPhoto(blob, caption, extraChatId) {
        const _send = async (chatId) => {
            try {
                ...
                return await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT}/sendPhoto`, {
                    method: 'POST', body: fd
                });
